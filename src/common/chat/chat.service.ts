...
import { RecipientType, NotificationCat, NotificationType } from '@prisma/client';
...
export class ChatService {
  ...
  async getOrCreateChat(data: CreateChatDto) {
    let chat = await this.prisma.chat.findUnique({
      where: {
        studentId_agentId_itemId: {
          studentId: data.studentId,
          agentId: data.agentId,
          itemId: data.itemId || '', // Wait, itemId is optional. But Prisma unique constraints with nulls can be tricky in Mongo. Let's findFirst instead.
        }
      } as any // Temporary bypass to fix the unique constraint logic below
    });
  ...
  async getChatById(chatId: string) {
    const chat = await this.prisma.chat.findUnique({
      ...
      include: {
        ...
        agent: { select: { id: true, firstName: true, lastName: true, companyName: true, profileImage: true } },
      }
    });
  ...
  async saveMessage(data: SendMessageDto) {
    ...
    const recipientId = data.senderType === 'STUDENT' ? chat.agentId : chat.studentId;
    const recipientType = data.senderType === 'STUDENT' ? RecipientType.AGENT : RecipientType.STUDENT;
  ...
  async markMessagesAsRead(chatId: string, recipientId: string, recipientType: 'STUDENT' | 'AGENT') {
